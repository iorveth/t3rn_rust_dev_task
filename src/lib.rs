...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		Executed { who: T::AccountId, execution_id: T::ExecutionId, encoded_bytes: Vec<u8> },

		Commited { who: T::AccountId, execution_id: T::ExecutionId, number_of_steps: u32 },

		Reverted { who: T::AccountId, execution_id: T::ExecutionId },
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn generate_side_effects_steps(
			...
			// Emit an event.
			Self::deposit_event(Event::Executed { who: account_id, execution_id, encoded_bytes });
		...
		pub fn commit(
			...
			let commited_side_effects =
				Self::do_commit(side_effects, number_of_commited_side_effects, number_of_steps)?;
			...
			// Emit an event.
			Self::deposit_event(Event::Commited { who: account_id, execution_id, number_of_steps });
		...
		pub fn revert(
			...
			ensure!(number_of_commited_side_effects > 0, Error::<T>::NothingToRevert);
			...
			// Emit an event.
			Self::deposit_event(Event::Reverted { who: account_id, execution_id });
	...
	impl<T: Config> Pallet<T> {
		...
		fn parse_side_effects(encoded_bytes: Vec<u8>) -> Vec<Vec<u8>> {
			encoded_bytes
				...
				.filter_map(
					|subslice| if subslice.is_empty() { None } else { Some(subslice.to_vec()) },
				)
				.collect()
		}
		...
		fn get_number_of_commited_side_effects(side_effects_vec: &SideEffectsVec<T>) -> usize {
			side_effects_vec.iter().filter(|side_effect| side_effect.is_commited()).count()
		}
		...
		fn parse_swap_execution_type(
			...
			Ok(ExecutionType::Swap { caller, to, amount_from, amount_to, asset })
		}
		...
		fn parse_multi_tran_execution_type(
			...
			Ok(ExecutionType::MultiTran { caller, to, amount, asset })
		}
