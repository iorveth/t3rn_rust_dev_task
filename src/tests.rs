...
fn it_works_for_default_value() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(TemplateModule::side_effects(execution_id).unwrap().into_inner(), side_effects);

		// Assert that the correct event was deposited
		System::assert_last_event(Event::Executed { who: 1, execution_id, encoded_bytes }.into());

		// Execute commit extrinsic
		let number_of_steps = 1;
		assert_ok!(TemplateModule::commit(RuntimeOrigin::signed(1), DEFAULT_ACTOR_ID, execution_id, number_of_steps));
		...
		// Assert that the correct event was deposited
		System::assert_last_event(Event::Commited { who: 1, execution_id, number_of_steps }.into());

		// Execute revert extrinsic
		assert_ok!(TemplateModule::revert(RuntimeOrigin::signed(1), DEFAULT_ACTOR_ID, execution_id));
		...
		// Assert that the correct event was deposited
		System::assert_last_event(Event::Reverted { who: 1, execution_id }.into());
	});
}
